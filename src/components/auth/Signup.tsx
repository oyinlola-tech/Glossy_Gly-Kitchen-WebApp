...
export const Signup: React.FC = () => {
  ...
  const completeSocialAuth = async (provider: 'google' | 'apple', token: string) => {
    const referralCode = formData.referralCode.trim() || undefined;
    const response = provider === 'google'
      ? await apiService.loginWithGoogle({ idToken: token, referralCode })
      : await apiService.loginWithApple({ identityToken: token, referralCode });
  ...
  const handleSocialSignup = async (provider: 'google' | 'apple') => {
    ...
    try {
      ...
      toast.success(`${provider === 'google' ? 'Google' : 'Apple'} sign-up successful`);
      navigate('/menu');
    } catch (error: any) {
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:block lg:w-1/2 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-12 z-20 bg-gradient-to-t from-black/80 to-transparent">
          <h2 className="text-4xl font-serif text-white mb-4">Join Our Culinary Family</h2>
          <p className="text-xl text-white/90">Create an account and unlock exclusive dining experiences</p>
        </div>
      </div>
